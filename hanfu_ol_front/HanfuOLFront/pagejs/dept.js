...
var userService = mvc.findService("dept");
rock.initBasicMethod(userService);
...
function loadTableData() {
	...
	$table.bootstrapTable({
		...
		queryParams: function (options) {
			...
			param.pageSize = options.pageSize;// 设置每页大小
			param.pageNum = options.pageNumber;// 设置当前页码
			param.sort = options.sortName + " " + options.sortOrder;// 设置排序规则字符串
			return param;
		},
...
function removeById(id) {
	userService[$.isArray(id)?"removeMore":"remove"]({ "id": id}, function (rtn, status) {
		if (rtn.hasError) {
...
function submitForm($frm, id) {
	...
	if (bv.isValid()) {
		...
		if (rock.isBlank(id)) {
			userService.add(vo, function (rtn, status) {
				if (rtn.hasError)
			...
			userService.update(vo, function (rtn, status) {
				if (rtn.hasError)
					alert(rock.errorText(rtn, "保存数据出错!"));
				else {
					modal.modal('hide');
